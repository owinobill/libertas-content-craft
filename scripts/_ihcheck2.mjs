...
const distDir=path.resolve('dist-debug'); const PORT=4201;
...
for(const route of ['/insights-hub']){
  const p=await b.newPage();
  const errs=[];
  p.on('console',m=>{if(m.type()==='error')errs.push(m.text().slice(0,3000));});
  p.on('pageerror',e=>errs.push('pageerror: '+e.message.slice(0,3000)));
